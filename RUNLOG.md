...
## [2026-09-24] v1.1.0 发布验收 + ISO 引用核实与更正

- **动作**: owner 在网页创建 Release；agent 经网关 **GET 复核**发布物，并完成 ISO 引用核实后的文档更新
- **机器状态披露（F8）**: 纯 GET / 文档操作，无训练

### 1. v1.1.0 Release 复核通过（网关 GET）

```
$ curl -sk --resolve api.github.com:443:172.23.192.1 .../releases/tags/v1.1.0
  tag       : v1.1.0
  name      : v1.1.0 — Scope & platform boundaries / 适用范围与平台边界
  published : 2026-09-23T05:27:24Z
  draft     : False
  assets    : []            ← 符合"无新模型资产"的声明
  body      : 8801 chars    ← 与仓库 docs/release_notes_v1.1.0.md 逐字一致（仅 CRLF/LF 差异）
```

**URL**: https://github.com/2572873335/car-rl/releases/tag/v1.1.0

正文完整性抽查（每个 token 在中英各出现一次 ⇒ 双语堆叠无损）：
`v ≤ 0.50`(×4，含表格) / `3300`(×2) / std 串 `1.40 3.98 4.40 4.45 5.17 6.13`(各×2)
/ `7.42/7.97/7.65`(×2) / `0.77`(×2) / `41,936`(×2) / `适用范围与平台边界`(×1)。

### 2. v1.0.0 本地 tag 对齐核查（owner 要求）

```
local  v1.0.0^{} = 84342bd472052e53eecdd0317ea254e4a00c5f86
remote v1.0.0    = 84342bd472052e53eecdd0317ea254e4a00c5f86
⇒ MATCH，无需任何操作
```
**结论**：`git fetch --tags` 取回的本地 tag **本就指向正确提交**，**未做任何轻推或改动**。
v1.0.0 的 release 页面**未触碰**。

### 3. ★ ISO 3691-4 引用核实（owner 执行，**原叙事数字有误**）

| 项 | 原叙事 | 核实结论 |
|---|---|---|
| 标准存在性/名称 | ISO 3691-4 | ✅ 属实（ISO 3691-4:2023，2020 首版 / 2023-06 更新；覆盖 AGV/AMR；CE 协调标准） |
| 速度-分离机制 | "速度与分离距离挂钩" | ✅ 成立，但**术语须改**：分区防护（警告场→减速、防护场→停止，场尺寸 `S=K×T+C` 随车速缩放）；**不叫 "SSM"**（那是 ISO/TS 15066 协作机器人的词） |
| **固定限速值** | **「约 ≤ 0.5 m/s」** | ❌ **标准中无此数**。唯一固定值是 **0.3 m/s**，且仅限**人员检测静默工况**（装卸对接） |

**⇒ 已更正 `docs/scenario_scope_statement.md` 与两份 README**（中英同步）：

- 叙事由「翻转」升级为**「精确命中」**——**0.3 m/s 落在本平台训练区间（0.25–0.50）之内**，
  每一句有据可查；
- **证据等级如实标注**：标准名称/版本/机制 = **一次源确认**；
  **0.3 m/s 数值 = 二手技术解读（置信度高，非原文直读）**，若论文作承重引用须补原文核对；
- **待核框的存在价值得到实证**：agent 无网络 ⇒ 未核 ⇒ 0.5 m/s 这一错误数字**被拦在文档之外**
  整整两天，直到 owner 核实才放行。**这正是铁律 1 待核机制的设计意图。**

- **验收结论**: v1.1.0 发布验收**通过**；v1.0.0 tag 一致性**通过（无需操作）**；
  **ISO 悬置项清零**；唯一未决 = **硬件下单（owner）**
- **产物**: `docs/scenario_scope_statement.md`（§5 引用框）、`README.md` / `README.zh-CN.md`（人机共存低速区段）

### 4. ⚠ 连带影响：已发布 release 正文中一句陈述已过时（**待 owner 定夺**）

已发布的 v1.1.0 正文「读者须知」第 1 条含：

> "The scope statement contains one externally-sourced reference that is **flagged
> as pending verification** within the document itself…"

该句**在发布时（2026-09-23）为真**，但本日 ISO 核实后待核框已移除，**如今描述仓库现状已不准确**
（后半句"未作承重引用"仍成立）。

**处置**：
- **本地 `docs/release_notes_v1.1.0.md` 保持不动**，以与已发布正文逐字一致（避免本地/远端分叉）；
- **修改已发布正文需 token，属 owner 动作**（若改，则为 2 分钟网页编辑）；
- **两种处理都站得住**：① 保持原样 = 正文是 v1.1.0 当天状态的**时间戳快照**，历史准确；
  ② 更新 = 与当前仓库现状一致。**agent 不擅自改已发布物**，提请 owner 定夺。

---

## [2026-09-25] Phase F — Step 0.5 预演闸门 **PASS** + 正例锚点建成

- **命令**:
  ```
  uv run python results/20260925_phaseF_step05/step05_train.py    # 训练 + gate1
  uv run python results/20260925_phaseF_step05/step05_gate2.py    # gate2（补跑，见下）
  uv run python results/20260925_phaseF_positive/positive_pole.py # 正例终验
  ```
- **铁律 4 豁免声明（按 `DATA_MANAGEMENT.md` §10.8 要求显式声明）**：
  Step 0.5 为 **1M 步 / 16 envs = 244 更新**，低于 300 下限——
  **正式豁免**：它是**预演闸门（smoke/probe）**，**不产出结论**。
  真正的训练（Step 1/2）**照常适用铁律 4**。
- **机器状态披露（F8）**: 各跑前 `pgrep` 确认为空
- **冻结文件**: 全部 `sha256 -c` 通过（`follow_env.py` 未动）

### 1. Step 0.5（Q4 重定义版）**双门全过**

```
gate 1 (std max < 2.0)         : PASS   std 1.000 → 1.403（max 1.433）, 245 updates
gate 2 (早停率 < 50%)          : PASS   5.0% (2/40)
         平均回合时长          : 19.72 s（t_max = 20.0）
         碰撞                  : 2/40
```

**⇒ 幅值轴可训**（计划 §1.2 的未测项已测）。**D0 的速度上界不适用于本轴**——
实测支持这一区分，**未越权类推**。

**实现瑕疵（已修）**：gate 2 首次运行崩溃于变量名笔误（`ev.t_` 应为 `ev.t`，
env 用 `self.t` 存时间）。**训练本身已完成并保存**，仅评估段崩溃；
已修 `step05_train.py` 并补跑 `step05_gate2.py`。

### 2. 正例（阈值锚点）建成并通过全部验收

**锁定配置**：`margin=0.05, safety=0.60, kp=0.8`
（**更正，review1 R2**：另有 `ythresh=0.20, yg=0.25, ymax=0.06, recover=0.998`
四个常量，经实测**全部无效（死代码）**——单步速度最大变化量 `a_max·DT ≤ 0.04`
而 `ythresh=0.20`，物理不可达，跨 11 族 × 3 间距触发 **0 次**。
**机制陈述更正**：本策略**不是**"预判-避让"，而是
「**P+FF + 一条按【达成】收距率与间隙相关包络比较就全力刹车**的规则」（触发率 2.4–8.1%）。
消融定论：只夹持命令 = 60/220 崩；只留全权刹车 = 0/220 全过。）

```
d=0.20  0/220 碰撞   名义 2.24 obs-cm（P+FF 2.54，比值 0.88×）
d=0.50  0/220 碰撞   名义 4.52 obs-cm（P+FF 4.52，比值 1.00×）
d=1.00  0/220 碰撞   名义 3.66 obs-cm（P+FF 3.66，比值 1.00×）
```

- 每档 **11 个攻击族**，含**两个打死基线的**（step-down 0.1s 打死 P+FF、0.5 Hz 打死 v1 RL）
  以及 held-out 族的同类波形；
- **`crawl` 项确认锚点不是靠"远避"过关**（巡航时仍跟踪到 1.23 obs-cm）；
- **owner 三项附加验收全过**：两个指定探针零碰撞、全族全档零碰撞、名义误差 ≤1.5×。

### 3. ★ 前四版失败的原因（**已定位，值得记录**）

**前四个手工候选在 step-down 上全部 14–20/20 撞车**，一度看起来像"无解"。
**forced-win 探针否证了该读法**：

```
零动作        0/20 撞车
P+FF         20/20 撞车
（同上但把 gap_max 钉死为 0.20，无追赶过程：仍 20/20）
```

轨迹显示：**gap 单调下降而跟随车始终快于领车** ⇒
**攻击惩罚的是"激进的收距"，不是"刹车能力不足"**。
初始 gap 取自 `U(0.20, 1.2)`，跟随车为追赶而积累收距速率，
领头阶跃时**无法及时卸掉**该速率；而**零动作永不撞车**——V2V 先验天然保距。

⇒ **约束是【达成】收距速率的上界**（review2 nit：须带「达成」二字，否则与命令夹持混淆），锚点据此设定上限：

```
c_max = safety · sqrt(2 · a_max_lo · (gap − COLLISION_GAP − margin))
```

`a_max_lo` 取 DR 下限 1.0（悲观侧成立）。**收距超限即全力刹车，否则退化为纯 P**
⇒ 名义精度不受影响，因为**包络只在激进时起作用**。

- **验收结论**: Step 0.5 **PASS** ⇒ 可进入 Step 1（攻击者训练，3 天硬门）；
  正例**建成且通过验收** ⇒ H-A3 阈值有锚（Phase C 的教训已免疫）
- **产物**: `results/20260925_phaseF_step05/{step05_train.py,step05_gate2.py,step05.log,step05_defender.zip}`、
  `results/20260925_phaseF_positive/{positive_pole.py,_out_pole_final.txt,正例 7 个迭代脚本}`
- **遗留 / 下一步**:
  - **正例须走评审**（owner Q6：锚点必须先被审过）；
...
## [2026-09-26] review3 后续：F1 独立复核确认 + 重复探针清理核对

- **动作**: 纯核验（无训练、无冻结文件改动）
- **机器状态披露（F8）**: 跑前 `pgrep -af 'uv run python'` 为空；本批 1920 回合，
  确定性事件计数，非耗时基准

### 1. review3 追加的 F1（拦截级）**独立复核：确认成立**

评审在 `_review3_out_E.txt` 中提出并追加为 F1：**判据未钉死 `d_des`**，
在 `d=0.50 / 1.00` 档上**冻结的未训练基线 v1 自己通过判据**。
作者以**独立脚本**（`_verify_review3_F1.py`，逐回合 `policy.reset()`，
B\* 判定域 B1–B5 + B6 三格，20 局/项）复测三档：

```
  d_des = 0.2    pole 0/160 err 4.28 | v1 32/160 err 13.17 | zero 0/160 err 104.21 | P+FF 80/160 err 23.41
  d_des = 0.5    pole 0/160 err 12.78| v1  0/160 err  3.36 | zero 0/160 err  60.39 | P+FF 42/160 err 19.56
  d_des = 1.0    pole 0/160 err 23.95| v1  0/160 err  4.00 | zero 0/160 err  65.12 | P+FF  4/160 err 23.35
```

- **核心结论确认**：**d=0.50 / 1.00 上 v1（未训练基线）0/160 撞车、err 3.36 / 4.00**，
  **自己通过 (b)(c)(e′)** ⇒ 该档上判据**不度量"增益"**（§10.5c 在另一维度的同型复发）。
- **更强的一点（作者补充）**：**正例在 d≥0.5 上反而劣于 v1**（12.78 vs 3.36；23.95 vs 4.00）
  ⇒ 若在那些档上判定，"手工锚点"会被自己的基线比下去，判据自相矛盾。
- **与评审数值的差异（如实披露）**：作者的 P+FF 撞车率更高（d=0.20：80/160 vs 评审 63/160），
  差异来自 harness 细节（作者每策略新建 env，评审复用）。**承重结论不受影响**。
- **⇒ 处置：`d_des = 0.20` 的钉死是必需的，不是可选**（已写入 plan §3 的 A1 条与 ASSUMPTIONS）。

### 2. 评审删除了两个探针文件——**核对为真重复，删除安全**

评审在交付时删除 `_review3_probe_C.py` 与 `_review3_run.py`，称其为逐字节重复的副本。
作者以 md5 核对：

```
_review3_probe_C.py (deleted)  = 539ff4c4...  ==  _review3_probe_support.py (kept)
_review3_run.py     (deleted)  = 1f8f0ab2...  ==  _review3_probe_zoo.py     (kept)
```

⇒ **确为逐字节重复**，保留版本已入库，删除不损失信息。**接受该清理。**

### 3. 评审如实披露的未完成项（**接受，不改判**）

- `_review3_probe_F.py`（`gapCap`/`asym`/`slew` 组合族）**未运行**；
  评审给出**有理由的预期**（该族为已测族的子情形，构造性死于 (e′)）并**以"未实测"记账**——
  **这是正确的诚实条款做法**，作者接受该记账，不据此判负。
- zoo 的 (e′) 用 **pooled** 口径（与 review1/2 一致）；逐项值在输出中但未逐项过门。
- 本轮全部 **20 局/项**；Step 3 的 headline 仍须走 **A-2 的 30 seed**。

- **验收结论**: F1 确认 ⇒ `d_des = 0.20` 钉死为**必需要求**（已落 plan/ASSUMPTIONS）；
  重复文件删除核对通过；评审未完成项已如实记录
