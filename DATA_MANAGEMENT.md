...
> 沉淀于 Phase C：把实验脚本拷进 `results/` **不等于**可复现。
> 本轮出现"主脚本已归档、其依赖仍在 `/tmp`"的情形，
> 导致 `results/` 里的结果**从干净检出跑不出来**。

**规则**：
1. 归档实验脚本时，**必须同时归档其依赖模块**；
2. 归档脚本**不得依赖 `/tmp` 等易失路径**（`/tmp` 副本还会造成
   陈旧模块污染——见 F18）；
3. **从归档位置实际跑一次**（或做导入测试）确认依赖齐全；
4. 已提供回归守卫 `results/20260920_phaseC_probe/scripts/
   archived_scripts_reproducible_test.py`：
   它拷贝仓库、移除 `/tmp` helper、断言脚本仍可加载。

**注意**：清理 `sys.path` 时须**同时处理被 import 的模块自身**——
若 helper 在 import 时也插入 `/tmp`，只改调用方无效（本轮踩过）。

### 10.5b 几何退化族与判据原语（**W6 教训**）

> 沉淀于 Phase C W6：**四版判据连续被同一个两行几何策略击穿**（F20）。
> 该赛道上"抢内圈 + 保持速度"是占优策略，**任何以"是否/多快取得领先"
> 为因变量的判据都必然被满足**。

**(a) 退化反例表必须常驻几何族。**
历史上击穿判据的是两行代码：
```python
[0.0, 1.0 if delta < 0.30 else -1.0]        # 领先就回外圈
[1.0, 1.0 if delta < 0.60 else -1.0]        # 提前切内圈
[1.0 if delta < 0.45 else -0.5, 1.0 if delta < 0.45 else -1.0]
```
**它们比 zero-action 危险得多**：zero-action 是"懒惰"（易被任何指标排除），
几何策略是"**聪明地投机**"——看起来在做事、指标漂亮、实则零博弈技能。
**任何新判据的退化表必须包含这三个实现**（现成代码见
`results/20260920_phaseC_probe/scripts/`）。

**(b) 判据代码必须继承 env 原语，禁止重实现。**
R4 的教训：`FULL` 自行实现"是否失去领先"，**丢掉了冻结 env 对套圈回绕的守卫**
（`overtake_env.py:229-231`：`ahead_flag` 永不复位，因 delta 在 ±L/2 回绕
"必须不算失去领先"），导致**做得更好的策略被判 0**（规则机 30/30 局完成
机动却得 FULL=0.00）。

**规则**：判据的胜负/成功/终止语义**一律复用被评估环境自身的定义**；
确需重实现时，必须**逐条抄写原语中的边界守卫**并加单测对照原实现。

### 10.5c 退化反例表必须含「随机/主动扰动」类

> 沉淀于 Phase C M1：击穿判据的不只是几何族（§10.5b），
> 还有一类**更隐蔽**的退化策略——**主动扰动指标**。

**两类的区别**：

| 类 | 行为 | 击穿方式 |
|---|---|---|
| **几何族**（§10.5b） | 静态地"聪明投机"（占内圈） | 指标**可被几何买到** |
| **随机/扰动类**（本节点） | **随机**或**盲目地做目标动作** | 指标**与目标脱钩**——随机做也得分 |

**实例（M1.3 判据，两次更正后仍被击穿）**：

```
  正例（反制脚本）          切入命中率 44.6%
  blind diver（零时机逻辑）  命中率 60.5%   ← 盲目做，比正例还高
  random diver（随机切）     命中率 84.6%   ← 随机做，近正例两倍
  只在"命中态"切入           完成率  0.00   ← 按指标做，从不获胜
```

**⇒ 判据与目标**反相关**：随机行为的"命中率"最高，而按判据行事者完成率为 0。**

**规则**：任何新判据的退化反例表**必须包含**：
1. **几何族**（§10.5b，三个现成实现）；
2. **随机/主动扰动类**——至少两个：
   - **random policy**（每步以固定概率执行目标动作，无策略）；
   - **blind policy**（**无条件**执行目标动作，零时机逻辑）。
   两者若**得分接近或高于正例**，该判据作废。

**并须独立检查一条因果假设**（本轮教训）：
> **"被判据测量的量"与"任务成功"是否真的相关？**
> 检验方式：**消融**——移除该能力后，成功率须**显著下降**。
> M1.3 的消融结果是"移除后完成率**上升**"（0.00 → 0.77），故判据作废。
> **"排除退化策略"（§10.4）不等于"与目标因果挂钩"**——本轮退化策略
> **恰好通过了** §10.4 的检验，却与目标脱钩。
...
### 10.9 commit 引用即落地（**文档-现实一致性**）

> **凡 commit message 引用某裁决 / 某 plan / 某文件，该文件必须在**同一 commit 内**
> 已存在或同 commit 修改。禁止「提交信息声称改了 X，而 X 未变」。**

**Why**：`c0b7b7d`（2026-09-22）声称给 `plan_D0_scenario_audit_v2.md` 加了「wrapper 硬约束」
与「§8」，但该 commit **只改了 `README.md` 与一个探针脚本**——plan 文件 0 次出现 "wrapper"，
全仓库 grep 不到 `D0.5`。根因：编辑脚本 `_d0_wrapper_mandate.py` 只存在于 `/tmp`，
目标文件在仓库，两副本未同步 ⇒ **编辑静默丢失，而提交信息按意图书写**。
后果：盘上 plan 与 owner 裁决**方向相反**，README 指向不存在的小节。

**How to apply**：提交前用 `git show --stat <commit>` 核对「声称改的文件是否真在改动列表里」；
对被引用的文件 grep 关键字（如 `grep -c wrapper <被引文件>`）。
**提交信息描述的是意图，不是事实**——这一条与 §10.6 诚实条款同源，
但针对的是**对仓库状态**的陈述，而非对实验数据的陈述。

> **编号说明**：owner 指示立为「§10.6」，但 §10.6 已被「诚实条款」占用，故编为 **§10.9**。
...
### 10.10 执行权边界（**灰色地带的标准动作**）

> **执行者只做不需要新授权的事；遇到需要凭据 / 授权 / 对外承诺的步骤，
> 精确定位并交还负责人，且在被拦下后不尝试任何变通。**
>
> 配套：**待核引用不得作为承重论据**——无网络/无权限核实时，
> 以醒目待核框标出，**不猜测、不留模糊**（本例见下）。

**范例（v1.1.0 发布，2026-09-23）**：agent 完成了全部不要凭据的步骤
（正文起草、双语文档、tag、推送、文档同步），**唯独创建 GitHub Release 需要鉴权**——
本机无 token（`gh auth status` 未登录、无 token 文件/env），实测未鉴权 POST 返回
**401 Requires authentication**。agent 的处理：

1. **精确交还**：说明"内容与 tag 已推，Release 本体需你执行"，并给出两条具体路径
   （网页建 / 提供最小权限 token）；
2. **拒绝变通**：排查过程中一次"扫描本机凭据文件"的操作被权限分类器拦下，
   判定为 *Credential Exploration*。**agent 未尝试任何绕过，并把该拦截如实上报**；
   原话"**找凭据不该是我自作主张做的事**"即本条的要义；
3. **边界收益实证**：owner 选择网页建（两分钟、零额外风险面），
   而"签发-保管-撤销 token"的风险面凭空消失。

**同轮的第二个范例（待核机制的实证价值）**：ISO 3691-4 引用中
「约 ≤ 0.5 m/s」一项**是错的**（标准中唯一固定限速为 **0.3 m/s**，且限静默工况）。
因 agent 无网络、按本条**标为待核而未采用**，该错误数字**被挡在文档之外两天**，
直到 owner 核实后才放行正确版本。**若当初"看着合理就写进去"，一个错误的标准引用
会进入对外发布物。**

> **编号说明**：§10.9 与 §10.8 的排列顺序在本文件中是颠倒的（10.9 在前）。
> 本条目顺延编为 §10.10，**未因此改动既有编号**（避免制造失效引用）。
