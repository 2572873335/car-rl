...
## 评审流程（plan → review → execute）

任何**新方向、新实验批次、环境改动、论文/报告修订**启动前，执行三步：

1. **Plan**：写一页纸方案——假设、方法、验收标准、时间预算、风险与预案；
2. **Review**：交给独立评审（另一 AI、同学或导师），要求逐条指出硬伤与隐患，
   评审意见连同响应一并存档于 `reviews/`（命名 `YYYYMMDD_<对象>_review<N>.md`）；
3. **Execute**：评审意见处理完毕后才允许动手；执行中的重大偏离回到 step 1。

已验证价值：本流程已拦截 4 次实质问题——论文格式/图表三轮（19 项）、
路线图离线 RL 库选型错误（sb3-contrib 无离线算法，避免 Phase A 返工 1-2 天）。

原则：评审者无执行权，执行者无豁免权；任何"感觉没问题"都必须让位给
"评审通过"。
