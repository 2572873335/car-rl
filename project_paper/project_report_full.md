...
**训练分布范围内**的域随机化下性能不退化。本文进一步以 30 seed 对照五种策略
（两种模仿实现、两种离线 RL、在线微调），揭示一个**能力-效率谱系**：离线 RL
（IQL）零交互即达满分，在线微调进一步压缩耗时，而模仿的瓶颈被定位于实现
而非数据。本文完整记录了 14 轮"失败—诊断—修复"的迭代过程，
...
> 本节回答 RQ2：**同一批演示数据下，模仿、离线 RL、在线微调三者的
> 数据效率与安全边界如何排序？** 原始数据见 `results/20260920_rq2_offline/`，
> 假设检验见 `research/ASSUMPTIONS.md`（H1–H7；其中 H1–H3、H6 的阈值训练前
> 登记，H4 阈值为事后修订、H7 为结果驱动追加，均在该文件显式标注）。

#### 4.8.1 实验设计

本节沿用全项目的严格对照协议：同一批演示数据（`demos_v1.npz`，300 局 /
41936 转移，规则状态机采集，seed 族 30000+i）、同一观测与动作接口、
**固定 30 seed（2000–2029）**、确定性策略、五类终止原因全审计
...
**表 4.5　五种策略的能力维度对照（30 seed，2000–2029，名义参数）**

| 方法 | 范式 | 数据使用 | 成功率 | 碰撞 | 冲出 | t_ot | mean_v |
|---|---|---|---|---|---|---|---|
| BC (manual) | 监督模仿 | 41936 演示 | 5/30 | 25 | 0 | 2.3 s | 0.48 |
| BC (d3rlpy) | 监督模仿 | 同左 | **30/30** | 0 | 0 | 2.10 s | 0.53 |
| IQL | **离线 RL** | 同左 | **30/30** | 0 | 0 | 2.10 s | 0.53 |
| TD3+BC | **离线 RL** | 同左 | **30/30** | 0 | 0 | 2.04 s | 0.26 |
| PPO 微调 | 在线 RL | BC 起点 + 5M 交互 | **30/30** | 0 | 0 | **1.37 s** | **0.70** |

> 域随机化下四方法成功率均为 30/30，无退化（manual-BC 6/30）。
> 原始输出（逐 seed 转录，可复跑）：`results/20260920_rq2_offline/final_table_30seed_transcript.txt`
> （由 `final_eval_30.py` 生成）。
> **两个"BC"是不同实现**（见 4.8.3）；TD3+BC 取**稳定版**（critic lr 1.5e-4，
> 见 4.8.3 末的表 4.8），与 4.8.2 表 4.6 的默认 lr 版本不同。
...
同 base seed 30000+i），每档独立训练。嵌套设计确保"数据效率"的因果链干净——
唯一变量是 N，不含抽样噪声。

![图 14 RQ2 数据效率曲线](figures_v2/图14_RQ2数据效率_v1.png)

**表 4.6　数据效率（成功率，10 seed / 取 N 集）**

| N（演示局数） | manual-BC | d3rlpy-BC | IQL | TD3+BC |
|---|---|---|---|---|
| 50 | 0/10 | 10/10 | **10/10** | 2/10 |
| 100 | 0/10 | 10/10 | **10/10** | 10/10 |
| 200 | 0/10 | 10/10 | **10/10** | 10/10 |
| 300 | 2/10 | 10/10 | **10/10** | 2/10 |

**H4 检验：成立。** IQL 在 **N=50** 即达 10/10（10 seed），而 manual-BC 在
N=300 仍仅 2/10——离线 RL 的数据效率高于论文版模仿实现。
（注：H4 的判定阈值在实验中途由"BC(N*)≤2/10"细化为"manual-BC(N*)<IQL(N*)"，
旨在区分两个不同的 BC 实现；两种判据下结论一致。已在 `research/ASSUMPTIONS.md`
标注为 post-hoc 修订。）

**一个意外发现（TD3+BC 的非单调）**：TD3+BC 在 N=50 与 N=300 均为 2/10，
中间两档却是 10/10。本表 TD3+BC 均为**默认 lr** 版本，其训练不稳定
（critic_loss 发散，见 4.8.3 表 4.8）与这一非单调一致，
说明 **TD3+BC 对优化路径高度敏感**：某些数据量下训练"恰好"收敛到好策略，
另一些则发散失败。IQL 全程稳定（critic_loss 收敛至 ~14），构成鲜明对照。

#### 4.8.3 实现敏感性：两个"BC"与失败机制

本节实验暴露一个方法论要点：**同一个算法名（BC）在两个实现下可以是
完全不同的策略**。
...
| **N=50 成功率** | **0/10** | **10/10** |
| **N=300 成功率** | **2/10** | **10/10** |

**H6 假设**：manual-BC 的成功率**不随演示数改善**（瓶颈是实现，非数据）。

**H6 检验：成立**（四档成功率 max−min = 2/10 ≤ 阈值 2/10；因结果恰在阈值
边界，按登记规则**补 seed 至 30**：max−min = 5/30 ≈ 1.67/10，仍 ≤ 2/10，
结论稳固，见 `results/20260920_rq2_offline/h6_boundary_30seed.txt`）。
manual-BC 在 50/100/200 局全部 **0/10（全体撞车）**，300 局才 2/10。逐位复现证据：
N=300 得 2/10、8 碰撞、2.2 s，且 BC loss = **1.7721**，与 `train_ot.py pretrain`
原始日志完全相同——证明补跑数据可信。

**结论**：同样 41936 条数据，一个实现（manual-BC）近乎崩溃，另一个
（d3rlpy-BC）50 条即满分。**行为克隆的瓶颈是实现，而非数据**。

**H7：失败机制定位**（限时实验，仅 d=300 一点 + 两变体）：

| 变体（N=300） | 成功率 | 碰撞 |
|---|---|---|
| 原版（高斯 NLL, lr 1e-3） | 2/10 | 8 |
| MSE 损失 | 2/10 | 8 |
| lr 减半 5e-4 | 0/10 | 10 |

**H7 检验：证伪。** 损失函数与学习率均非根因。按限时纪律停止（未蔓延成
超参搜索），机制如实记为"多因素"，留作 future work。

**TD3+BC 的稳定性（表 4.8）**。上文多处引用"TD3+BC 默认 lr 下训练发散"，
此处给出证据。同一数据（d=300）、同一架构，仅 critic/actor 学习率不同：

**表 4.8　TD3+BC 学习率敏感性（d=300，10 seed）**

| 配置 | 末期 critic_loss | 成功率 | 碰撞 | t_ot | mean_v |
|---|---|---|---|---|---|
| 默认 lr 3e-4 | **发散至 ~2.2e6** | 2/10 | 8 | 8.7 s | 0.35 |
| lr 1.5e-4（稳定版） | **459** | **10/10** | 0 | 2.0 s | 0.26 |

默认 lr 下 critic 值函数发散，策略退化为 2/10；学习率减半后 critic 稳定、
策略达满分。**这是"离线 RL 对优化超参敏感"的直接证据**，与 IQL 的平稳
（critic_loss 收敛至 ~14，表 4.5 的 IQL 行）形成对照。故 4.8.1 表 4.5 采用
**稳定版（lr 1.5e-4，t_ot 2.04 s、mean_v 0.26）**；默认 lr 版的非单调现象
（表 4.6）另见 4.8.2。

#### 4.8.4 讨论：能力-效率谱系

综合表 4.5–4.7，三种范式构成清晰的**能力-效率谱系**：

1. **演示数据决定策略的能力上限**：IQL（纯离线、零交互）在 30/30 试次上
   全部成功（成功率 100%，Wilson 95% 置信下界 ≈ 89%），说明**演示数据
   本身已足以支撑接近最优的策略**，无需任何在线交互；
2. **在线交互决定效率天花板**：PPO 微调把 t_ot 从 2.1 s（离线满分策略）
   压到 **1.4 s**（提速 33%），代价是 5M 步环境交互；
3. **模仿的数据效率最低**：manual-BC 即便用全部 41936 条演示仍仅 5/30——
   但这一"低效"根源于**模仿机制**（4.8.3 的 H6/H7），而非数据不足。

**对 4.5 节 LfD 叙事的精化**：4.5 节观察到"BC 2/10 不足以单独部署，需 RL 接管"。
本节进一步**定位其根因**：BC 的薄弱源于模仿机制本身，与数据量无关。
这**不削弱 LfD 的价值**，反而强化：PPO 从 manual-BC 这个"坏初始化"（5/30）
出发仍收敛到 30/30，说明 **LfD 管线的鲁棒性来自 RL 的修复能力**，对 BC
预热质量不敏感——这正是工程上最有价值的安全性。同时，本节揭示
**"BC→RL" 并非唯一补救路径**：更强的模仿实现（d3rlpy-BC）或离线 RL（IQL）
无需在线交互即可达满分，为算力受限场景提供了替代方案。
...
**5.4.1 鲁棒性的连续刻画（Phase A 补充）** 上文只覆盖"域随机化开/关"两点
（1.00→1.06 cm）。本节将其展开为**连续扰动曲线**：令执行器参数按退化方向
连续偏离（τ 增大、a_max 减小，ε∈{0,10,20,30,33,40,50}%），对 RL（final）
与规则 P+FF 各评估 **50 个固定 seed**，主指标为无碰撞率。

**表 5.5　执行器退化下的鲁棒性（50 seed，跟车任务）**

| ε | τ (s) | a_max (m/s²) | RL 无碰撞 | RL mean\|e\| | P+FF 无碰撞 | P+FF mean\|e\| |
|---|---|---|---|---|---|---|
| 0% | 0.120 | 1.50 | 50/50 | 1.21 cm | 50/50 | 2.10 cm |
| 10% | 0.132 | 1.35 | 50/50 | 1.21 cm | 50/50 | 2.10 cm |
| 20% | 0.144 | 1.20 | 50/50 | 1.25 cm | 50/50 | 2.11 cm |
| 30% | 0.156 | **1.05** | **47/50** | 1.30 cm | 50/50 | 2.14 cm |
| 33% | 0.160 | **1.00（DR 下界）** | **41/50** | 1.36 cm | 50/50 | 2.16 cm |
| 40% | 0.168 | 0.90 | **37/50** | 1.43 cm | 50/50 | 2.21 cm |
| 50% | 0.180 | 0.75 | **27/50** | 1.50 cm | 50/50 | 2.33 cm |

**读法与对 5.4 节结论的修正**。连续曲线揭示的比"开/关"两点丰富得多：

1. **RL 有失效边界，P+FF 没有**。RL 从 ε=30%（τ=0.156、a_max=1.05，**仍在
   DR 训练分布内**）即开始出现稀有失效（3/50），此后随 ε 增大**单调加速
   恶化**（3→9→13→23/50）；P+FF 全程 50/50 零碰撞。→ **原假设被证伪
   （对应 plan 的 H2）**：RL 并非"退化斜率更小"，而是存在明确的失效边界。
2. **但 RL 的绝对精度全程更优**：50% 档 RL mean|e|=1.50 cm 仍优于 P+FF 的
   2.33 cm（约 1.5×）。两种方法**鲁棒性画像互补**——RL 更准但会失稳，
   P+FF 永不失效但不精确。
3. **分布内外分段**：ε≤20%（分布内）RL 50/50 零失败；ε=30%（DR 边界内侧、
   a_max=1.05）已现稀有失效 3/50；ε=33% 时 a_max=1.005（**仍 ≥ DR 下界 1.0，
   尚在分布内**）失效 9/50；ε≥40%（a_max=0.90，**首次跌破 DR 下界 1.0，
   进入分布外**）失败率继续上升。这给出 RQ4 的核心答案：**域随机化提供
   的是"分布内"鲁棒性；分布外退化不可由 DR 预测，存在明确的失效边界——
   且边界略早于分布外缘（在分布内侧即已出现稀有失效）。**

失效模式可解释：撞车全发生在**大初始间距的追赶场景**（gap0≈0.7–0.8 m），
此时 a_max 降至 0.75（低于训练下限），策略"指令所需的制动/加速物理上做
不到"。这一发现提示 future work：给 RL 输出加一层**物理可行性护栏**（当
a_max 不足时禁止超出可行制动幅度的指令），理论上可兼得 RL 的精度与规则的
安全。
...
| 图 14 | `图14_RQ2数据效率_v1.png` | RQ2 数据效率曲线（4 方法） | `data_efficiency_sweep.txt` |
