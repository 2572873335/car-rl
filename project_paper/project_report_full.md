...
### 4.8 模仿、离线 RL 与在线微调：一个能力-效率谱系

> **本节状态**：骨架（2026-09-20）。数字一律以 `【待填：来源】` 占位，
> 待数据效率扫描与 30-seed 终版评估完成后回填（A6 阶段）。
> 原始数据：`results/20260920_rq2_offline/`；假设检验：`research/ASSUMPTIONS.md`。

#### 4.8.1 实验设计

本节沿用全项目的严格对照协议：同一批演示数据（`demos_v1.npz`，300 局 /
41936 转移，规则状态机采集，seed 族 30000+i）、同一观测与动作接口、
**固定 10 seed（2000–2009）**、确定性策略、五类终止原因全审计
（collision / offtrack / lost / success / failed）。

**离线叙事的底线**：模仿与离线 RL 的**训练过程不与环境交互**——训练脚本
只读 `demos_v1.npz`，仅评估时构造环境。此约束由代码结构强制
（`rq2_offline.py`：`train` 不 import env，`eval` 单独）。

**五行对照表**（能力维度：【待填：results/20260920_rq2_offline/eval_*.txt，30-seed 终版】）：

| 方法 | 范式 | 数据使用 | 成功率 | 碰撞 | 冲出 | t_ot | mean_v |
|---|---|---|---|---|---|---|---|
| BC (manual) | 监督模仿 | 41936 演示 | 【待填】 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
| BC (d3rlpy) | 监督模仿 | 同左 | 【待填】 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
| IQL | **离线 RL** | 同左 | 【待填】 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
| TD3+BC | **离线 RL** | 同左 | 【待填】 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
| PPO 微调 | 在线 RL | BC 起点 + 5M 交互 | 【待填】 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |

> 注：两个 BC 为**不同实现**（见 4.8.3）；TD3+BC 取稳定版
> （critic lr 1.5e-4，见 4.8.3 脚注）。

#### 4.8.2 数据效率

**H4 假设**：IQL 达到满分所需演示数 ≤ manual-BC 达到其最佳所需演示数
（离线 RL 的数据效率不低于模仿）。

**方法**：四个**嵌套子集**（50/100/200/300 局，均取 `demos_v1.npz` 前 N 集，
同 base seed），每档独立训练。嵌套设计确保"数据效率"的因果链干净——
唯一变量是 N，不含抽样噪声。

**曲线图**：【待填：`fig_rq2_data_efficiency.png`，四条曲线
manual-BC / IQL / TD3+BC（d3rlpy-BC 作参考线）】

**原始数据**（`results/20260920_rq2_offline/data_efficiency_sweep.txt`）：

| N | manual-BC | d3rlpy-BC | IQL | TD3+BC |
|---|---|---|---|---|
| 50 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
| 100 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
| 200 | 【待填】 | 【待填】 | 【待填】 | 【待填】 |
...
#### 4.8.3 实现敏感性：两个"BC"的对照

本节实验意外暴露一个方法论要点：**同一个算法名（BC）在两个实现下可以是
完全不同的策略**。

| | manual-BC（本文 4.5 节） | d3rlpy `BCConfig` |
|---|---|---|
| 网络 | MLP [128,128]（PPO 头） | d3rlpy 默认 encoder |
| 目标 | 高斯 NLL（mean + variance） | 连续动作默认确定性 MSE |
| 优化 | 手动 Adam lr 1e-3，10 epoch | d3rlpy 训练循环 |
| d=50 成功率 | 【待填】 | 【待填】 |
| d=300 成功率 | 【待填】 | 【待填】 |

**H6 假设**：manual-BC 的成功率**不随演示数改善**（瓶颈是实现，非数据）。

**H6 检验**：【待填：成立/证伪】。
逐位复现证据：manual-BC d=300 得 2/10、8 碰撞、2.2s，BC loss = 1.7721，
与 `train_ot.py pretrain` 原始日志完全相同——补跑数据可信。

**H7：失败机制定位**（损失函数 vs 学习率）：

| 变体（d=300） | 成功率 | 碰撞 |
|---|---|---|
| 原版（高斯 NLL, lr 1e-3） | 【待填】 | 【待填】 |
| MSE 损失 | 【待填】 | 【待填】 |
| lr 减半 5e-4 | 【待填】 | 【待填】 |

**H7 检验**：【待填：成立/证伪】。若两者均未修复 → 排除单一超参，
如实报告"多因素"，机制留作 future work。

#### 4.8.4 讨论：能力-效率谱系

**（本段三段论已定型，待数字回填后确认措辞）**：

演示数据决定了策略的**能力上限**（IQL 纯离线即满分），而在线交互决定的是
**效率天花板**（PPO 把 t_ot 压到 【待填】）。三种范式构成清晰的
**能力-效率谱系**：

1. **BC（manual）数据效率最低**——复合误差使其即使有数据也学不出可用策略；
2. **IQL / TD3+BC 免交互达满分**（【待填：成功率】），但非最快；
3. **在线 PPO 最快**（t_ot 【待填】），代价是需要环境交互。

**对 4.5 节 LfD 叙事的精化**：4.5 节观察到 "BC 2/10 不足以单独部署，
需 RL 接管"——本节进一步定位其根因：**BC 的薄弱源于模仿机制本身**
（4.8.3 的 H6/H7），而非演示数据不足。这**不削弱 LfD 的价值**，反而强化：
RL 从"坏初始化"出发仍收敛到 10/10，说明**LfD 管线的鲁棒性来自 RL 的修复能力**，
对 BC 预热质量不敏感——这正是工程上最有价值的安全性。
