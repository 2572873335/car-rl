...
> 假设检验见 `research/ASSUMPTIONS.md`（H1–H7，全部训练前登记）。
...
|---|---|---|---|---|---|---|---|
| BC (manual) | 监督模仿 | 41936 演示 | 5/30 | 25 | 0 | 2.3 s | 0.50 |
| BC (d3rlpy) | 监督模仿 | 同左 | **30/30** | 0 | 0 | 2.1 s | 0.53 |
| IQL | **离线 RL** | 同左 | **30/30** | 0 | 0 | 2.1 s | 0.53 |
| TD3+BC | **离线 RL** | 同左 | **30/30** | 0 | 0 | 2.0 s | 0.26 |
| PPO 微调 | 在线 RL | BC 起点 + 5M 交互 | **30/30** | 0 | 0 | **1.4 s** | **0.70** |

> 域随机化下四方法成功率均为 30/30，无退化（manual-BC 6/30）。
> 原始输出：`results/20260920_rq2_offline/final_table_30seed.txt`。
> **两个"BC"是不同实现**（见 4.8.3）；TD3+BC 取稳定版（critic lr 1.5e-4，见 4.8.3）。
...
**H4 检验：成立。** IQL 在 **N=50** 即达 10/10，而 manual-BC 在 N=300
仍仅 2/10——离线 RL 的数据效率显著高于论文版模仿实现。

**一个意外发现（TD3+BC 的非单调）**：TD3+BC 在 N=50 与 N=300 均为 2/10，
中间两档却是 10/10。这与其训练不稳定（critic 发散，见 4.8.3）一致，
...
1. **演示数据决定策略的能力上限**：IQL（纯离线、零交互）即达 30/30 满分，
   说明**演示数据本身已足以支撑最优策略**，无需任何在线交互；
...
1. **RL 有失效边界，P+FF 没有**。RL 从 ε=30%（τ=0.156、a_max=1.05，**仍在
   ...
   恶化**（3→9→13→23/50）；P+FF 全程 50/50 零碰撞。→ **H2 被证伪**：RL
   并非"退化斜率更小"，而是存在明确的失效边界。
2. **但 RL 的绝对精度全程更优**：50% 档 RL mean|e|=1.50 cm 仍优于 P+FF 的
   2.33 cm（约 1.5×）。两种方法**鲁棒性画像互补**——RL 更准但会失稳，
   P+FF 永不失效但不精确。
3. **分布内外分段**：ε≤30%（分布内）RL 零失败、误差平稳；ε≥33%（a_max
   跌破 DR 下界 1.0）失败率单调上升。这给出 RQ4 的核心答案：**域随机化
   提供的是"分布内"鲁棒性；分布外退化不可由 DR 预测，存在明确的失效边界。**
