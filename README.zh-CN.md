...
**为什么低速带重要。** ISO 3691-4:2023 是无人驾驶工业车辆（AGV/AMR）的
国际安全标准，它要求车辆对人员实施**分区速度适应**：警告场触发减速、防护场触发
安全停止，且防护场尺寸随车速与系统响应时间缩放。标准语境中**唯一的固定限速是
0.3 m/s**，且仅适用于人员检测被静默的工况（如装卸站对接）。该数值**落在本平台的
验证包络（0.25–0.50 m/s）之内**，因此本文所覆盖的正是紧密跟随需求最强的安全敏感
工况带；具体场站限速仍属风险评估范畴，标准不设统一数字。
*（标准名称、版本与分区机制为一次源确认；0.3 m/s 数值来自二手技术解读——
置信度高，但非原文直读。）*
